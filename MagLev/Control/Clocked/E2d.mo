...
block E2d "Calculate position from hall sensor signal"
  ...
  Modelica.Clocked.ClockSignals.Interfaces.ClockInput clock annotation (Placement(transformation(
        ...
        rotation=90,
        origin={0,-120})));
  Modelica.Clocked.RealSignals.Sampler.SampleClocked sample_i annotation (Placement(transformation(extent={{-90,50},{-70,70}})));
...
equation
  ...
  connect(i, sample_i.u) annotation (Line(points={{-120,60},{-92,60}}, color={0,0,127}));
  connect(clock, sample_e.clock) annotation (Line(
      points={{0,-120},{0,-80},{-60,-80},{-60,-12}},
      color={175,175,175},
      pattern=LinePattern.Dot,
      thickness=0.5));
  connect(clock, sample_i.clock) annotation (Line(
